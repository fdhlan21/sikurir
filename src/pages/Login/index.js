...
export default function Login({navigation, route}) {
  ...
  const sendData = async () => {
    if (kirim.username.length == 0) {
      ...
      // pertama-tama cek data di storage lokal
      const localResult = await loginLocal();
      if (localResult.status == 200) {
        setLoading(false);
        await storeData('user', localResult.data);
        toast.show('Login berhasil (dari data lokal)', { type: 'success' });
        navigation.replace('Home');
        return;
      }

      // jika tidak ada di lokal, coba ke server
      try {
        ...
        console.log('Server login failed, fallback ke lokal:', error);

        // fallback: gunakan hasil localResult (sudah dicek di atas, tapi pastikan)
        setLoading(false);
        if (localResult.status == 200) {
          await storeData('user', localResult.data);
          navigation.replace('Home');
          ...
          toast.show(localResult.message || 'Gagal login, periksa koneksi atau akun Anda');
        }
      }
    }
  };
  ...
  return (
    <View
      ...
      <View
        ...
        {/* tombol "kembali ke halaman pilihan" dihapus */}
      </View>
    </View>
  );
}